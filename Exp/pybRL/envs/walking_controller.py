...
class WalkingController():
    ...
    def set_gamma_value(self, factor = 1):
        if(self.gait == "trot" or self.gait == "bound" or self.gait == "walk"):
            self.gamma = PI/2
        elif(self.gait == "sidestep_left"):
            self.gamma = PI
    ...
    def set_leg_gamma(self):
        if(self.gait == "turn_left" ):
            ...
            self.back_right.gamma = self.gamma
            self.back_left.gamma =-(PI- self.gamma)
            ...
            # self.front_right.gamma = self.gamma*right_turn + PI/2*left_turn
            # self.front_left.gamma = self.gamma*left_turn + PI/2*right_turn
            # self.back_right.gamma = (PI - self.gamma)*right_turn + PI/2*left_turn
            # self.back_left.gamma = (PI - self.gamma)*left_turn + PI/2*right_turn
            

        

    def transform_action_to_motor_joint_command3(self, theta, action, plane_angle_weight):
        ...
        self.set_gamma_value(plane_angle_weight)
        self.set_leg_gamma()
        ...
        for leg in legs:
            ...
            leg.z = leg.r * np.cos(leg.gamma)*(np.cos(leg.theta) + leg.b*(left_turn-right_turn)*0)
            # print(leg.name)
            # print(leg.z)
            #leg.z = np.abs(leg.z)
            ...
            leg.motor_abduction = (np.arctan2(leg.z, -leg.y))
