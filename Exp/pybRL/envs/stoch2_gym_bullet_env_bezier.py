...
class Stoch2Env(gym.Env):
	...
	def _get_reward(self,action,energy_spent_per_step,cost_reference):
		...
		current_w = round(self.avg_omega_per_cycle,1)
		desired_v  = self.dv
		desired_w = self.dw
		...
		reward = (np.exp(-40*((current_v - desired_v)**2)) + np.exp(-50*((current_w - desired_w)**2)))/2 - 0.01 * energy_spent_per_step
