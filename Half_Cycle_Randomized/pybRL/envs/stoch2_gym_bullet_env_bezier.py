...
class Stoch2Env(gym.Env):
	...
	def _get_reward(self,action,energy_spent_per_step,cost_reference):
		...
		reward = (np.exp(-35*((current_v - desired_v)**2)) + np.exp(-1.0*((current_w - desired_w)**2)))/2 - 0.01 * energy_spent_per_step #0.9
	...
	def generateRandomV(self):
		w = round(random.uniform(0,2.5),2)
		v = round(random.uniform(0.25,0.5),2)
		return v,w
	...
	def GetObservation(self):
		...
		if self._gait == "trot+turn":
			...
			obs = np.concatenate(([self.dv],[self.dw], motor_angles)).ravel()
		return obs
