...
class Stoch2Env(gym.Env):
	...
	def __init__(self,
		...
		self.termination_steps =  20
		self.intervals = [4, 7, 11, 15, 19]
	...
	def do_simulation(self, action, n_frames, callback=None):
		...
		avg_vel = 0
		avg_omega = 0
		omega = 2 * np.pi * self._frequency * frequency_weight #commented for the time being
	...
	def linearvelocityf(self):
		...
		if x < vx[0]:
			...
			y2 = vy[2]
			v = self.linear_func(x, x1, y1, x2, y2)
		elif x >= vx[2] and x <= vx[3]:
			v = vy[2]
		elif x > vx[3] and x < vx[4]:
			x1 = vx[3]
			y1 = vy[3]
			x2 = vx[4]
			y2 = vy[4]
	...
	def _get_reward(self,action,energy_spent_per_step,cost_reference):
		...
		current_v =  round(self.avg_vel_per_cycle,2)                   #self.GetAvgCurrentVelocity()
		current_w = round(self.avg_omega_per_cycle,2)
		...
		reward = (np.exp(-30*((current_v - desired_v)**2)) + np.exp(-0.9*((current_w - desired_w)**2))) - 0.01 * energy_spent_per_step
		#reward = np.exp(-0.9*((current_w - desired_w)**2))# - 0.01 * energy_spent_per_step
